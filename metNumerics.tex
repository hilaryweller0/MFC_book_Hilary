...
models use the numerical methods they do. We begin with early finite\nobreakdash-difference
approaches and the problem of fast waves, then show how elliptic solves,
grid staggering, and semi\nobreakdash-implicit methods emerged. We
then turn to modern challenges: parallel computing, quasi\nobreakdash-uniform
spherical meshes, and high\nobreakdash-order methods such as spectral
elements and discontinuous Galerkin.
...
\end{equation}
\item Can you animate the solution? What can you say about the part of the
solution that started from the smooth initial conditions? What do
the discontinuities do to the solution? Why?
\item Next try setting the Courant number to be 1.1 and animate the solution
...
for two reasons because air density varies by orders of magnitude
between the surface and the stratosphere. Also, numerical solutions
including (\ref{eq:ico}) are more difficult because you cannot just
march forward; you have to find solutions for the components of the
vector $\mathbf{u}$ that satisfy the momentum equation (\ref{eq:mom})
and simultaneously satisfy (\ref{eq:ico}). The wave acoustic wave
speed has actually been made infitite! Charney's solution will be
described in section \ref{subsec:ellipticSolutions}. First, attempt
this exercise exploring time step restrictions due to fast waves.

\subsection{Example: Numerical Solution of Gravity Waves}\label{subsec:solveSW}
...
to a numerical method for solving an elliptic equation. By making
the incompressible approximation (\ref{eq:ico}) for the primitive
equations, acoustic waves have infinite speed. This converts a time
marking problem into a constrain problem, and makes the equations
elliptic. \citet{Char48} derived an elliptic vorticity equation to
represent some slowly evolving weather regimes. This was an equation
for the vertical component of the total vorticity:
\begin{equation}
...
or Coriolis parameter, $f$, is the planetary vorticity. To derive
...
wind is close to geostrophic balance, which means that, in the momentum
equation, the Coriolis term approximately balances horizontal pressure
gradients, and the wind flows along isobars, rather than down pressure
gradients:
\begin{equation}
\begin{array}{cccc}
...
in the Northern hemisphere. The winds advect (move) the pressure,
making it evolve slowly, making the weather predictable. However geostrophic
balance cannot be used to predict the weather. Geostrophic balance
(\ref{eq:geoBalance}) implies incompressibility and, substituting
(\ref{eq:geoBalance}) into the momentum equation (\ref{eq:mom})
results in the wind and other variables never changing. This is a
persistence forecast - the weather tomorrow will be the same as today.
We don't need equations to make a persistence forecast. So it seems
that we need need some way to ignore the ``entirely irrelevant gravity
motions'' \citet{Char49}. \citet{CFvN50} made the first numerical
...
\addcontentsline{toc}{subsubsection}{Mathematical Exercises}
\begin{enumerate}
...
\addcontentsline{toc}{subsubsection}{Coding Exercise}
...
weather forecasts, all of these processes need to be included, but
without the time step being limited by the speed of the fast waves.
This is where semi-implicit time stepping comes in. Rather than just
simplifying the equations, the equations are written as corrections
to simplified equations, so that the simplified, linear parts can
be solved implicitly, leaving the corrections, which may be non-linear
or just other slow terms, to be solved explicitly. 
...
at location $j$.
\end{enumerate}
\todo[inline]{Maybe just use implicit to solve the linearised eqns
and compare with previous exercise?}
...
more realistic and the models became more similar to those that are
used now. From the 1950s to the 1990s huge progress was made and numerical
weather prediction made dramatic progress, becoming ever more valuable
and accurate to longer lead times.

\citet{Smag58} solved the the primitive equations in three dimensions
(with just two vertical levels). But the paper mostly discusses causes
of instability and no plots of numerical results are presented, suggesting
that there was little realism in the results. \citet{Phil59} simulated
...
viscosity to mimic the effects of turbulent diffusion. \citet{Smag63}
mentioned Arakawa's emerging work on energy conserving methods which
would prove revolutionary in atmosphere and ocean modelling. The first
UK Met Office numerical forecast was produced in 1965 on a Comet computer
\citep{MOhistory26} which solved a three-level vorticity equation
and used geostrophically balanced initial winds in order to avoid
noisy initial conditions \citep{BW61}. Arakawa's work was necessary
in order to move from vorticity equations to momentum equations, which
...
They wrote the momentum equation, for the horizontal velocity, $\mathbf{v}$,
...
is the horizontal component of the gradient. Solving the vector-invariant
...
The two-dimensional linearised shallow water equations can be obtained
...
instantaneously. It is essential that gravity waves are represented
in order to adjust the fluid to geostrophic balance, but it is less
important to accurately model the speed of gravity waves.
...
\todo[inline]{Write a C-grid and an A-grid shallow water model?}
...
\addcontentsline{toc}{subsubsection}{Spectral Method Exercises}
...
we find a departure point, which won't usually fall on a grid point,
and interpolate from the old values onto the departure point. This
enables long, stable and accurate time steps, but the properties transported
are not conserved. This means that there can be spurious sources and
sinks of mass, energy, moisture, or anything else that is transported
by the model. 
...
\addcontentsline{toc}{subsubsection}{Semi-Lagrangian Exercise}
...
as much by computer architecture as by numerical analysis.

In the 1970s to 1990s, numerical weather prediction used mainframe
computers such as the vector-parallel Cray computers. Parallel means
that they had more than one processor, running in parallel. In the
1990s, Crays had 2, 4, 8 or of 16 processors. The atmosphere was decomposed
into a domains, one for each processor, and the equations of motion
were solved separately on each domain, with communication at the boundaries.
...
model \citep{CDM+97,DCM+05}. SISL enables excellent weather and climate
prediction on latitude-longitude grids since SISL overcomes time step
restrictions near the poles. At time of writing the operational model
used by the UK Met Office, EndGame, still uses SISL on a latutide-longitude
grid, and has consistently been one of the most accurate models, usually
second only to ECMWF \citep[eg.][]{HJV+24}. However the combination
of further parallelisation, increased resolution and the convergence
of meridians towards the poles means that parallelisation progress
on latitude-longitude grids is stuck -- more processors are added,
domains get smaller and the code does not run faster or even at the
same speed using higher resolution. It is not possible to overcome
the large amount of communication that must be done near the poles
to make latitude-longitude usable at modern high resolutions. 
...
efficiently, but not tens of thousands \citep{WBB+17}, which is needed
for the most accurate, highest resolution solutions. In order to scale
in parallel on 10s or 100s of thousands of processor, quasi-uniform
meshes of the sphere are needed, which do not have convergence of
points towards the poles, or any other locations.
...
down. This century, weather prediction has been moving away from the
latitude-longitude grid due (in part) to the parallel scaling bottle
necks as the number of parallel processors increases. Even before
parallel scaling was a critical bottleneck, ECMWF moved to a reduced
grid, which coarser resolution towards the poles \citep{HS91} in
order to save in computation time and memory requirement. This was
more straightforward using a spectral model as they key point was
to use few wavenumbers close to the poles. 
...
\ref{fig:grids}) and meshes of triangles are quasi-uniform and can
be constructed to be orthogonal. For these meshes, orthogonality needs
a different definition. The triangular icosahedron is the dual of
the hexagonal icosahedron. This means that if you put a point at the
centre of each polygon of the hexagonal icosahedron, and join the
points over the edges, then you get the triangular icosahedron. If
the mesh and its dual have edges that cross at right angles, then
the mesh is orthogonal. Orthogonal meshes have advangages in terms
of accuracy and efficiency, but there are other considerations, such
as the skewness -- do the edges of the dual grids bisect each other
or cross not at the edge centres \citep{Wel14}. There are compromises
to be made when optimising icosahedral meshes for numerical methods.

\section{Numerical Methods for Quasi-Uniform Meshes}
...
\item Implement PPM to solve the linear advection equation in 1D (give the
equation for a uniform 1d grid).
\end{enumerate}

\subsection{Finite Volume A- and C-grids for unstructured meshes}
...
a strong motivation to move to more complex numerical methods for
...
of meshes, and are extremely efficient on massively parallel computers.
Fourth-order accurate spectral elements are used in a version of NCAR
Community Atmosphere Model, CAM-SE \citep{NRC+10,ELM+13} on a cubed
sphere (fig. \ref{fig:CAMSE}, right). Considering just the horizontal
discretisation in CAM-SE, the surface of the Earth is divided into
quadrilateral elements. The solution is approximated on each element
using a basis function which is a two-dimensional, third-order polynomial.
A key advantage of spectral elements is that they use Lagrange interpolation
\citep{WTB+07} which means that the basis functions passes through
nodes on each element exactly and the weights of each basis function
are cheap to compute and avoid global communication. By placing these
notes at Gauss-Legendre-Lobatto quadrature points (fig. \ref{fig:CAMSE},
left) the order of accuracy is one more than the degree of the polynomial
used, so the third-order polynomial basis functions give a fourth-order
...
is slightly more expensive. The cost is offset by the excellent scaling
properties on massively parallel computers. DG is not currently being
used operationally for weather prediction but powerful research models
exist such as NUMA-NEPTUNE, \citep{KG12} developed at the Naval Postgraduate
School and DG-COSMO \citep{SBB+14}, a version of COSMO, which is
a limited area model that was used by Deutscher Wetterdienst before
ICON was operational.

\subsection{Mixed Finite Elements}

\todo[inline]{SE and DG are co-located. This motivates mixed finite
elements/compatible finite elements.}
...
\begin{itemize}
\item Fast waves impose severe stability constraints.
