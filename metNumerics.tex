...
\caption{Figure 4 from \citet{LJTN10} showing ``(left) A graphical illustration
...
the cubed-sphere faces''. Reproduced with permission from John Wiley
and Sons. Copyright 2010 by the American Geophysical Union.}\label{fig:CAMSE}
\end{figure}

The spectral element method enables arbitrarily high-order accuracy
on a variety of meshes, and is extremely efficient on massively parallel
computers. Fourth-order accurate spectral elements are used in a version
of NCAR Community Atmosphere Model, CAM-SE \citep{NRC+10,ELM+13}
on a cubed sphere (fig. \ref{fig:CAMSE}, right). Considering just
the horizontal discretisation in CAM-SE, the surface of the Earth
is divided into quadrilateral elements. The solution is approximated
on each element using a basis function which is a two-dimensional,
third-order polynomial (to achieve fourth-order accuracy). A key advantage
of spectral elements is that it uses Lagrange interpolation \citep{WTB+07}
which means that the basis functions passes through nodes on each
element exactly and the weights of each basis function are cheap to
compute and avoid global communication. By placing these notes at
Gauss-Legendre-Lobatto quadrature points (fig. \ref{fig:CAMSE}, left)
the order of accuracy is one more than the degree of the polynomial
used. 
...
solutions. Spectral element methods instead rely on small amounts
of diffusion to maintain stability and smooth solutions \citep{GTU+14}.

\subsubsection{Discontinuous Galerkin}

The discontinuous Galerkin (DG) method is similar to the spectral
...
SE and DG are co-located. This motivates mixed finite elements

\section{Is Machine Learning the Future of Weather and Climate Prediction}

ML weather forecasts are great. Current ML models drift and usually
diverge to an unrealistic state within a few weeks. Lots of work to
do here.
